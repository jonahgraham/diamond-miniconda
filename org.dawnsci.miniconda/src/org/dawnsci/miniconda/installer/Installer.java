...
package org.dawnsci.miniconda.installer;
...
import org.dawnsci.miniconda.MinicondaActivator;
...
public final class Installer implements IInstaller {
	...
	public void runManualInstall() throws CoreException {
		...
		try {
			...
			Status status = new Status(IStatus.ERROR, MinicondaActivator.PLUGIN_ID, "Anaconda Installer failed to launch.",
					e);
	...
	public void runInstall(String installPath, final IProgressMonitor monitor,
		...
		if (PlatformUtils.isWindowsPlatform()
			...
			if (new File(exe).exists()) {
				...
				Status status = new Status(IStatus.ERROR, MinicondaActivator.PLUGIN_ID,
						"Installation failed. Expected to find python here: '" + exe + "' but it was missing.");
				throw new CoreException(status);
			}
		} else if (monitor.isCanceled()) {
			Status status = new Status(IStatus.INFO, MinicondaActivator.PLUGIN_ID,
			...
			Status status = new Status(IStatus.ERROR, MinicondaActivator.PLUGIN_ID, string);
			throw new CoreException(status);
		}
	...
	private Process launchProcess(String[] cmdarray, InstallOutputHandler handler) throws CoreException {
		...
		try {
			...
			Status status = new Status(IStatus.ERROR, MinicondaActivator.PLUGIN_ID, "Failed to launch installer", e);
			throw new CoreException(status);
		}
	}
	...
	public static IPath getInstallerLocation() throws CoreException {
		...
		try {
			URL bundleURL = MinicondaActivator.getBundleURL(new Path("installer/miniconda_installer.txt"));
			if (bundleURL != null) {
				...
				URL installerUrl = MinicondaActivator.getBundleURL(new Path("installer/" + filePath));
